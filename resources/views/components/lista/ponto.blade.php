<div class="relative py-20 mb-20  bg-white dark:bg-gray-900">
    ...
    <div class="inline-flex w-full">
            ...
            <button  wire:click="create()" id="dropdownRadioButton"
                ...
                <tbody>
                    ...
                    @foreach ($lista as $key => $row)
                        <tr
                            ...
                            <td class="px-6 py-4">
                             @if(isset($row->prestador)) {{$row->prestador->nome}} @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($row->tipo == 'E')
                                <span class="text-green-700">Entrada</span>
                                @else
                                <span class="text-red-700">Saída</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($row->data_hora)->format('d/m/Y H:i') }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $row->tempo_trabalho }}
                            </td>
